...
class _SystemService extends Service {
    ...
    static {
        Service.register(this, this.events, this.props);
    }
    ...
    private _cpu = {
        usage: new Variable(0, {
            listen: [
                ...
                (out) => {
                    if (out.includes('all')) {
                        const usage = 100.0 - parseFloat(out.split(/\s+/).pop()!);
                        if (usage !== this.cpu_usage) {
                            this.onChange('cpu-usage', usage);
                        }
                        return usage;
                    } else return this.cpu_usage ?? 0;
                }
            ]
        })
    };
            ...
            listen: [
                ...
                (out) => {
                    if (!out.includes('kbmemfree') && out.length > 2) {
                        const usage = parseFloat(out.split(/\s+/)[4]);
        ...
        const usage = new Variable(0, {
            listen: [
                ...
                (out) => {
                    ...
                    if (match) {
                        const usage_v = parseFloat(match[1]);
                        if (usage_v !== old) {
